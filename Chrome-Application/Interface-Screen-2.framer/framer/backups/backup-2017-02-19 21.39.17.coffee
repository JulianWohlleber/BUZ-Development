...
Events.wrap(window).addEventListener "keydown", (event) ->
	...
	if sceneSwitcher is 1
		...
		updateTrends(Color_Regional, Trends_Regional)
		...
		updateTrends(Color_Fortress, Trends_Fortress)
		...
		updateTrends(Color_Robotic, Trends_Robotic)
		...
		updateTrends(Color_Virtual, Trends_Virtual)
		remove(Description_Regional, Description_Fortress, Description_Robotic)
		remove(Title_Regional, Title_Fortress, Title_Robotic)

updateTrends = (color, trends) ->
	if Trends_Regional is true
		Trends_Regional.destroy()
	if Trends_Fortress is true
		Trends_Fortress.destroy()
	if Trends_Robotic is true
		Trends_Robotic.destroy()
	if Trends_Virtual is true
		Trends_Virtual.destroy()
	
	print "hello", trends
	
	if mySuperLayer is Trends_Regional
		Trends_Regional = new Layer
			backgroundColor: myTransparent
	else if mySuperLayer is Trends_Fortress
		Trends_Fortress = new Layer
			backgroundColor: myTransparent
	else if mySuperLayer is Trends_Robotic
		Trends_Robotic = new Layer
			backgroundColor: myTransparent
	else if mySuperLayer is Trends_Virtual
		Trends_Virtual = new Layer
			backgroundColor: myTransparent
