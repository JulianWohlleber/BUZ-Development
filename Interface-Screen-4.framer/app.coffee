...
dotAnimation = new Animation activeDot,
	x: Screen.width
	options:
		delay: 1
		time: 0
...
updateTrends = (color, amountOfTrends, mySuperLayer) ->
	...
	dotAnimationNumber = 1
	dotAnimation.start()
	activeDot.onAnimationEnd ->
		print "test"
		if dotAnimationNumber < (amountOfTrends)
			dotAnimationNumber++
			print dotAnimationNumber, "animation-number"
			print dotAnimation.properties.x
			dotAnimation.properties.x = (Screen.width - horizontalMargin - (dotSpace * (amountOfTrends - dotAnimationNumber)))
			dotAnimation.start()
		else
			dotAnimationNumber = 1
			dotAnimation.properties.x = (Screen.width - horizontalMargin - (dotSpace * (amountOfTrends - dotAnimationNumber)))
